...
CAN_ERROR CAN_SetupAFLUT(LPC_CANAF_TypeDef *CANAFx, AF_SectionDef *AFSection) {
  ...
  if (AFSection->FullCAN_Sec == NULL) {
    ...
    for (i = 0; i < (AFSection->FC_NumEntry); i++) {
      ...
      if ((CANAF_FullCAN_cnt & 0x00000001) == 0) {
        ...
        CANAF_FullCAN_cnt++;
        if (CANAF_FullCAN_cnt ==
            AFSection->FC_NumEntry) // this is the lastest FullCAN entry
          count++;
      } else {
      ...
      AFSection->FullCAN_Sec =
          (FullCAN_Entry *)((uint32_t)(AFSection->FullCAN_Sec) +
                            sizeof(FullCAN_Entry));
    }
  }
  ...
  if (AFSection->SFF_Sec != NULL) {
    for (i = 0; i < (AFSection->SFF_NumEntry); i++) {
      ...
      if ((CANAF_std_cnt & 0x00000001) == 0) {
        ...
        CANAF_std_cnt++;
        if (CANAF_std_cnt ==
            AFSection->SFF_NumEntry) // if this is the last SFF entry
          count++;
      } else {
        buf = LPC_CANAF_RAM->mask[count];
        ID_temp = (buf >> 16) & 0xE7FF;
      ...
      AFSection->SFF_Sec =
          (SFF_Entry *)((uint32_t)(AFSection->SFF_Sec) + sizeof(SFF_Entry));
    }
  }
  ...
  if (AFSection->SFF_GPR_Sec != NULL) {
    for (i = 0; i < (AFSection->SFF_GPR_NumEntry); i++) {
      ...
      if (CANAF_gstd_cnt != 0) {
        buf = LPC_CANAF_RAM->mask[count - 1];
        ID_temp = buf & 0xE7FF;
        if ((ctrl1 != ctrl2) || (lowerSID > upperSID) ||
            (ID_temp > ((ctrl1 << 13) | lowerSID))) {
          return CAN_AF_ENTRY_ERROR;
        }
      }
      entry = (ctrl1 << 29) | (dis1 << 28) | (lowerSID << 16) | (ctrl2 << 13) |
              (dis2 << 12) | (upperSID << 0);
      LPC_CANAF_RAM->mask[count] = entry;
      CANAF_gstd_cnt++;
      count++;
      AFSection->SFF_GPR_Sec =
          (SFF_GPR_Entry *)((uint32_t)(AFSection->SFF_GPR_Sec) +
                            sizeof(SFF_GPR_Entry));
    }
  }
  ...
  if (AFSection->EFF_Sec != NULL) {
    for (i = 0; i < (AFSection->EFF_NumEntry); i++) {
      ...
      count++;
      AFSection->EFF_Sec =
          (EFF_Entry *)((uint32_t)(AFSection->EFF_Sec) + sizeof(EFF_Entry));
    }
  }
  ...
  if (AFSection->EFF_GPR_Sec != NULL) {
    for (i = 0; i < (AFSection->EFF_GPR_NumEntry); i++) {
      ...
      if (CANAF_gext_cnt != 0) {
        buf = LPC_CANAF_RAM->mask[count - 1];
        //				EID_temp = buf & 0x0FFFFFFF;
        if ((ctrl1 != ctrl2) || (lowerEID > upperEID) ||
            (buf > ((ctrl1 << 29) | (lowerEID << 0)))) {
          return CAN_AF_ENTRY_ERROR;
        }
      }
      ...
      LPC_CANAF_RAM->mask[count++] = entry;
      CANAF_gext_cnt++;
      AFSection->EFF_GPR_Sec =
          (EFF_GPR_Entry *)((uint32_t)(AFSection->EFF_GPR_Sec) +
                            sizeof(EFF_GPR_Entry));
    }
  }
...
CAN_ERROR CAN_LoadExplicitEntry(LPC_CAN_TypeDef *CANx, uint32_t id,
  ...
  total = ((CANAF_FullCAN_cnt + 1) >> 1) + CANAF_FullCAN_cnt * 3 +
          ((CANAF_std_cnt + 1) >> 1) + CANAF_gstd_cnt + CANAF_ext_cnt +
          (CANAF_gext_cnt << 1);
  ...
  if (format == STD_ID_FORMAT) {
    ...
    if (CANAF_std_cnt == 0) {
      ...
      if (((LPC_CANAF_RAM->mask[cnt2] >> 16) & 0xE7FF) > id) {
        LPC_CANAF_RAM->mask[cnt2] =
            (LPC_CANAF_RAM->mask[cnt2] >> 16) | (id << 16);
      } else {
        LPC_CANAF_RAM->mask[cnt2] =
            (LPC_CANAF_RAM->mask[cnt2] & 0xFFFF0000) | id;
      }
    } else {
      ...
      if (cnt1 == bound1) {
        ...
        bound1 =
            ((CANAF_FullCAN_cnt + 1) >> 1) + ((CANAF_std_cnt + 1) >> 1) - 1;
  ...
  /*********** Add Explicit Extended Identifier Frame Format entry *********/
    ...
    cnt1 = ((CANAF_FullCAN_cnt + 1) >> 1) +
           (((CANAF_std_cnt + 1) >> 1) + CANAF_gstd_cnt);
    cnt2 = 0;
...
CAN_ERROR CAN_LoadFullCANEntry(LPC_CAN_TypeDef *CANx, uint16_t id) {
  ...
  LPC_CANAF_RAM->mask[cnt1] = LPC_CANAF_RAM->mask[cnt1 + 1] =
      LPC_CANAF_RAM->mask[cnt1 + 2] = 0x00;
  cnt1 += 3;
  while (bound1--) {
  ...
  if (format == STD_ID_FORMAT) {
    ...
    if (CANAF_gstd_cnt == 0) {
      ...
      while (cnt1 < bound1) {
        ...
        while (
            (LPC_CANAF_RAM->mask[cnt1] >> 29) <
            (entry1 >> 29)) // increase until meet greater or equal controller
          cnt1++;
        buf0 = LPC_CANAF_RAM->mask[cnt1];
        if ((LPC_CANAF_RAM->mask[cnt1] >> 29) >
            (entry1 >> 29)) // meet greater controller
        {
    ...
    if (CANAF_gext_cnt == 0) {
      ...
      while (cnt1 < bound1) {
        while ((LPC_CANAF_RAM->mask[cnt1] >> 29) <
               tmp) // increase until meet greater or equal controller
          cnt1++;
        buf0 = LPC_CANAF_RAM->mask[cnt1];
        buf1 = LPC_CANAF_RAM->mask[cnt1 + 1];
        if ((LPC_CANAF_RAM->mask[cnt1] >> 29) >
...
CAN_ERROR FCAN_ReadObj(LPC_CANAF_TypeDef *CANAFx, CAN_MSG_Type *CAN_Msg) {
  ...
  if (interrut_word != 0) {
    ...
    for (msg_idx = head_idx; msg_idx <= tail_idx; msg_idx++) {
      ...
      if (test_bit) {
        pSrc = (uint32_t *)(LPC_CANAF->ENDofTable + LPC_CANAF_RAM_BASE +
                            msg_idx * 12);
