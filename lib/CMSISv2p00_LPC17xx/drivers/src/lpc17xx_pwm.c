...
void PWM_Init(LPC_PWM_TypeDef *PWMx, uint32_t PWMTimerCounterMode,
  ...
  if (PWMTimerCounterMode == PWM_MODE_TIMER) {
    ...
    PWMx->CTCR |=
        (PWM_CTCR_MODE((uint32_t)pCounterCfg->CounterOption)) |
...
void PWM_ConfigCapture(LPC_PWM_TypeDef *PWMx,
  ...
  CHECK_PARAM(PARAM_PWMx(PWMx));
  CHECK_PARAM(
      PARAM_PWM1_CAPTURE_CHANNEL(PWM_CaptureConfigStruct->CaptureChannel));
  ...
  if (PWM_CaptureConfigStruct->RisingEdge == ENABLE) {
    ...
    PWMx->CCR &=
        (~PWM_CCR_CAP_RISING(PWM_CaptureConfigStruct->CaptureChannel)) &
        PWM_CCR_BITMASK;
  }
  ...
  if (PWM_CaptureConfigStruct->FallingEdge == ENABLE) {
    ...
    PWMx->CCR &=
        (~PWM_CCR_CAP_FALLING(PWM_CaptureConfigStruct->CaptureChannel)) &
        PWM_CCR_BITMASK;
  }
  ...
  if (PWM_CaptureConfigStruct->IntOnCaption == ENABLE) {
    ...
    PWMx->CCR &=
        (~PWM_CCR_INT_ON_CAP(PWM_CaptureConfigStruct->CaptureChannel)) &
        PWM_CCR_BITMASK;
  }
}
...
void PWM_MultiMatchUpdate(LPC_PWM_TypeDef *PWMx, PWM_Match_T *MatchStruct,
  ...
  for (i = 0; i < 7; i++) {
    if (MatchStruct[i].Status == SET) {
      if (i < 4)
        *((volatile unsigned int *)(&(PWMx->MR0) + i)) =
            MatchStruct[i].Matchvalue;
      else {
        *((volatile unsigned int *)(&(PWMx->MR4) + (i - 4))) =
            MatchStruct[i].Matchvalue;
      }
      LatchValue |= (1 << i);
    }
