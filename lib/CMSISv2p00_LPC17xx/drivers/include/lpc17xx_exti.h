 ...
 */
typedef enum {
  EXTI_POLARITY_LOW_ACTIVE_OR_FALLING_EDGE, /*!< Low active or falling edge
                                            sensitive depending on pin mode */
  EXTI_POLARITY_HIGH_ACTIVE_OR_RISING_EDGE  /*!< High active or rising edge
                                            sensitive  depending on pin mode */
} EXTI_POLARITY_ENUM;
 ...
 */
typedef struct {
  EXTI_LINE_ENUM EXTI_Line; /*!<Select external interrupt pin (EINT0, EINT1,
                               EINT 2, EINT3) */
  ...
  EXTI_POLARITY_ENUM
      EXTI_polarity; /*!< 	If EXTI mode is level-sensitive: this element
                        use to select low or high active level if EXTI mode is
