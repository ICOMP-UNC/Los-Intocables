...
#define GPDMA_TRANSFERTYPE_M2P                                                 \
  ((1UL)) /**< Memory to peripheral - DMA control */
#define GPDMA_TRANSFERTYPE_P2M                                                 \
  ((2UL)) /**< Peripheral to memory - DMA control */
#define GPDMA_TRANSFERTYPE_P2P                                                 \
...
/*********************************************************************/ /**
                                                                         ...
                                                                         *registers
                                                                         **********************************************************************/
#define GPDMA_DMACCxControl_TransferSize(n)                                    \
  (((n & 0xFFF) << 0)) /**< Transfer size*/
#define GPDMA_DMACCxControl_SBSize(n)                                          \
  (((n & 0x07) << 12)) /**< Source burst size*/
#define GPDMA_DMACCxControl_DBSize(n)                                          \
  (((n & 0x07) << 15)) /**< Destination burst size*/
...
#define GPDMA_DMACCxControl_Prot1                                              \
  ((1UL << 28)) /**< Indicates that the access is in user mode or privileged   \
                   mode*/
#define GPDMA_DMACCxControl_Prot2                                              \
  ((1UL << 29)) /**< Indicates that the access is bufferable or not            \
                   bufferable*/
#define GPDMA_DMACCxControl_Prot3                                              \
  ((1UL << 30)) /**< Indicates that the access is cacheable or not cacheable*/
#define GPDMA_DMACCxControl_I                                                  \
  ((1UL << 31)) /**< Terminal count interrupt enable bit */
...
#define GPDMA_DMACCxConfig_SrcPeripheral(n)                                    \
  (((n & 0x1F) << 1)) /**< Source peripheral*/
#define GPDMA_DMACCxConfig_DestPeripheral(n)                                   \
  (((n & 0x1F) << 6)) /**< Destination peripheral*/
#define GPDMA_DMACCxConfig_TransferType(n)                                     \
...
#define PARAM_GPDMA_BSIZE(n)                                                   \
  ((n == GPDMA_BSIZE_1) || (n == GPDMA_BSIZE_4) || (n == GPDMA_BSIZE_8) ||     \
   (n == GPDMA_BSIZE_16) || (n == GPDMA_BSIZE_32) || (n == GPDMA_BSIZE_64) ||  \
...
#define PARAM_GPDMA_STATCLR(n)                                                 \
  ((n == GPDMA_STATCLR_INTTC) || (n == GPDMA_STATCLR_INTERR))

/* Macros check GPDMA request select type */
#define PARAM_GPDMA_REQSEL(n)                                                  \
  ((n == GPDMA_REQSEL_UART) || (n == GPDMA_REQSEL_TIMER))
/**
 * @}
 */
...
typedef struct {
  ...
  uint32_t TransferWidth; /**< Transfer width - used for TransferType is
                             GPDMA_TRANSFERTYPE_M2M only */
  uint32_t SrcMemAddr; /**< Physical Source Address, used in case TransferType
                          is chosen as GPDMA_TRANSFERTYPE_M2M or
                          GPDMA_TRANSFERTYPE_M2P */
  uint32_t DstMemAddr; /**< Physical Destination Address, used in case
                          TransferType is chosen as GPDMA_TRANSFERTYPE_M2M or
  ...
  uint32_t
      TransferType; /**< Transfer Type, should be one of the following:
                    ...
                    */
  uint32_t SrcConn; /**< Peripheral Source Connection type, used in case
                    TransferType is chosen as GPDMA_TRANSFERTYPE_P2M or
                    GPDMA_TRANSFERTYPE_P2P, should be one of following:
                     ...
                     */
  uint32_t DstConn; /**< Peripheral Destination Connection type, used in case
                    TransferType is chosen as GPDMA_TRANSFERTYPE_M2P or
                    GPDMA_TRANSFERTYPE_P2P, should be one of following:
                     - GPDMA_CONN_SSP0_Tx: SSP0, Tx
                     - GPDMA_CONN_SSP0_Rx: SSP0, Rx
                     - GPDMA_CONN_SSP1_Tx: SSP1, Tx
